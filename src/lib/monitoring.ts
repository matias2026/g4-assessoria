...
// Sem "use server": este módulo é importado tanto pelo lado do treinador
// (cockpit/monitoring-actions.ts, que exige requireCoachOrAdmin antes de
// chamar) quanto pelo lado do próprio aluno (dashboard/profile-actions.ts,
// que exige a sessão dele mesmo). A função daqui não faz nenhuma checagem
// de autorização — quem chama é responsável por isso antes de repassar o
// studentId/userId certo.
...
/**
 * Resumo de treinos concluídos — combina as duas fontes reais de dado que
 * existem hoje: `treinos` (upload manual de .FIT, atividade da Strava
 * anexada ao dia, ou conclusão só com RPE — ver dashboard/profile-actions.ts
 * e dashboard/strava-actions.ts) e `strava_activities` (toda atividade
 * sincronizada, mesmo sem treino prescrito naquele dia — ver
 * syncStravaNow). Uma atividade que já virou treino do dia não conta duas
 * vezes: fica de fora da segunda fonte pra não duplicar a sessão. Usada
 * tanto pelo treinador (qualquer aluno, ver cockpit/monitoring-actions.ts)
 * quanto pelo próprio aluno vendo os dados dele mesmo (ver
 * dashboard/profile-actions.ts) — cada chamador cuida da própria
 * autorização antes de chegar aqui.
 */
export async function computeMonitoringSummary(studentId: string, userId: string | null): Promise<MonitoringSummary> {
  ...
  if (treinoResult.error) {
    console.error("[monitoring] erro ao buscar treinos concluídos:", treinoResult.error.message);
  }
